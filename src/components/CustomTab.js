...
function TemplateEditor({ template, onClose, onToast }) {
  useBackButtonClose(() => onClose(false));
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {filteredEntries.map((e) => (
              <button
                ...
                className={`anim-in text-left glass-card rounded-xl p-3.5 hover:border-gold-500/40 flex gap-3 items-start relative w-full min-w-0 overflow-hidden ${
                  e.pinned ? "border-gold-500/40" : ""
                }`}
              >
                <button
                  onClick={(evt) => toggleEntryPin(e, evt)}
                  className="absolute top-2 right-2 text-sm z-10 shrink-0"
                ...
                <div className="min-w-0 flex-1 pr-6">
                  <h3 className="font-display text-cream font-semibold truncate min-w-0">{e.title}</h3>
                  <p className="text-cream/50 text-xs mt-1 line-clamp-2 break-words">{e.main_text || "বিস্তারিত দেখতে ক্লিক করো"}</p>
                  {e.tags?.length > 0 && (
